...
| <a name="input_castai_api_token"></a> [castai\_api\_token](#input\_castai\_api\_token) | Optional CAST AI API token created in console.cast.ai API Access keys section. Used only when `wait_for_location_ready` is set to true | `string` | `""` | no |
| <a name="input_castai_api_url"></a> [castai\_api\_url](#input\_castai\_api\_url) | URL of CAST AI API to be used while making cluster checks. | `string` | `"https://api.cast.ai"` | no |
...
| <a name="input_wait_for_location_ready"></a> [wait\_for\_location\_ready](#input\_wait\_for\_location\_ready) | Optional wait for location to be ready before finishing the module execution.  This option requires `castai_api_token` to be set | `bool` | `false` | no |
